...
from utils.utils import UniverseUtils, notif
import os
...
class SimulatedUniverse(UniverseUtils):
    ...
    def route(self):
        ...
        while True:
            if self._stop:
                break
            hwnd = pygetwindow.getActiveWindow()  # 根据当前活动窗口获取句柄
            Text = hwnd.title()
            warn_game = False
            cnt = 0
            while Text != "Honkai: Star Rail Honkai: Star Rail" and not self._stop:
                self.lst_changed = time.time()
                if self._stop:
            ...
            """
            if begin and not self.check("f", 0.4016, 0.3935) and not self.check("abyss/1",0.8568,0.6769):
                begin = 0
                self.press("F4")
                time.sleep(0.6)
                self.get_screen()
    ...
    def normal(self):
        ...
        if self.check("choose_bless", 0.9031, 0.9222):
            ...
            chose = 0
            self.battle = 0
            if self.check("reset",0.3557,0.0926):
                for _ in range(14):
                    self.get_screen()
                    img_down = self.check("z", 0.5, 0.2556, mask="mask", large=False)
                    if (
                        ...
                        time.sleep(0.2)
                        break
                    if not self.check("choose_bless", 0.9031, 0.9222):
                        return 1
                    time.sleep(0.2)
                self.get_screen()
                img_up = self.check("z", 0.5, 0.5406, mask="mask_bless", large=False)
                ...
                if res_up[1] == 2:
                    self.click(self.calc_point((0.5, 0.5406), res_up[0]))
                    chose = 1
                elif res_down[1] == 2:
                    self.click(self.calc_point((0.5, 0.2556), res_down[0]))
                    chose = 1
                if not chose:
                    self.click((0.3911, 0.0870))
                    time.sleep(1.2)
            # 未匹配到优先祝福，刷新祝福并再次匹配
            if not chose:
                for _ in range(8):
                ...
                if res_up[1] == 2:
                    ...
                    self.click(self.calc_point((0.5, 0.5406), res_up[0]))
            self.click((0.1380, 0.0870))
            tm=time.time()
            while time.time()-tm<1.6 and self.check("choose_bless", 0.9031, 0.9222):
                time.sleep(0.1)
                self.get_screen()
        ...
        # F交互界面
            ...
            if self.check("f", 0.4016, 0.3935, mask="mask_f1"):
                for _ in range(4):
                    ...
                    if text is None:
                        img = self.check(
                            "z", 0.2891, 0.3926, mask="mask_f", large=False
                        )
                        text = self.ts.sim_list(self.tk.interacts, img)
                    if text is not None:
                        break
        ...
        if self.isrun():
            ...
            if self.big_map_c == 0:
                ...
                if self.find:
                    ...
                    if self.floor in [0, 5]:
                        ...
                        while True:
                            self.exist_minimap()
                            # cv.imwrite("test_loc_scr.jpg",self.loc_scr)
            ...
            if self.mini_state:
                ...
                self.get_direc()
            return 2
        elif self.check("init", 0.8885,0.8046):
            if self.end:
                time.sleep(1)
                self.press('esc')
                self._stop = 1
            ...
            if not con:
                if self.diffi == 5:
                    ...
                    time.sleep(0.2)
                self.click((0.9375, 0.8565 - 0.1231 * (self.diffi - 1)))
            self.click((0.1260, 0.0861))
            if con:
                self.get_level()
            else:
                self.floor = 0
            ...
            if self.check("team4", 0.4786, 0.5954):
                ...
                for i in self.order:
                    self.click(
                        (0.9026 - dx * ((i - 1) % 3), 0.7740 - dy * ((i - 1) // 3))
                    )
                    time.sleep(0.3)
            ...
            time.sleep(0.6)
            self.get_screen()
            img = self.check("z", 0.5, 0.3231, mask="mask_fate", large=False)
            # cv.imwrite("test.jpg",img)
            res = self.ts.split_and_find([self.fate], img)
            self.click(self.calc_point((0.5, 0.3231), res[0]))
        elif self.check("fate_3", 0.9061, 0.9231):
            if not self.click_text(['2星祝福','奇物']):
                self.click((0.5, 0.5))
            self.click((0.5, 0.0889))
            time.sleep(1)
        # 事件界面
        elif self.check("event", 0.9322, 0.9074):
            # 事件界面：选择
            if self.check("arrow", 0.2140, 0.5000, mask="mask_event"):
            ...
            # 事件选择界面
                ...
                time.sleep(0.3)
                self.get_screen()
                if success and self.check("confirm", 0.2140, 0.5000, mask="mask_event", threshold=0.965):
                    self.click((self.tx, self.ty))
                else:
                    self.click((tx, ty))
                    time.sleep(0.3)
                ...
                for _ in range(7):
                    self.get_screen()
                    if not self.check("event", 0.9322, 0.9074):
                        break
                    time.sleep(0.1)
                self.lst_changed = time.time()
            else:
        ...
        # 选取奇物
        elif self.check("strange", 0.9198, 0.9212):
            time.sleep(0.6)
            self.get_screen()
            img = self.check("z", 0.5000, 0.7580, mask="mask_strange", large=False)
            res = self.ts.split_and_find(self.tk.strange, img, mode="strange")
            self.click(self.calc_point((0.5000, 0.7580), res[0]))
            self.click((0.1365, 0.1093))
            self.wait_fig(lambda:self.check("strange", 0.9198, 0.9212), 1.4)
        # 丢弃奇物
        elif self.check("drop", 0.9214, 0.9231):
            self.click((0.4714, 0.5500))
            self.click((0.1339, 0.1028))
            self.wait_fig(lambda:self.check("drop", 0.9214, 0.9231), 1.4)
        elif self.check("drop_bless", 0.9151, 0.9203, threshold=0.95):
            time.sleep(1.5)
            ...
            self.re_enter()
        elif self.check("enhance", 0.9031, 0.9231):
            self.quit = time.time()
            time.sleep(1.5)
            for i in [None, (0.7547, 0.5917), (0.6172, 0.5917)]:
                self.get_screen()
                if self.check("enhance_fail", 0.1312, 0.0815):
                    break
                if i is not None:
                    self.click(i)
                    time.sleep(0.3)
                self.click((0.1312, 0.0815))
                time.sleep(0.3)
                tm = time.time()
                self.get_screen()
                while not self.check("enhance", 0.9031, 0.9231) and time.time()-tm<7:
                    self.click((0.2062, 0.2054))
                    time.sleep(0.3)
                    self.get_screen()
            self.press("c")
            tm = time.time()
            while time.time()-tm<2 and not self.check("f", 0.4016, 0.3935, mask="mask_f1") and not self.isrun():
                self.get_screen()
                time.sleep(0.15)
            # time.sleep(0.35)
            # self.mouse_move(-30)
    ...
    def show_map(self):
        ...
        if not self._stop:
            if not self.debug_map.shape[0] == 8192:
                # continue
