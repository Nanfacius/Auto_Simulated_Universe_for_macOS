...
class UniverseUtils:
    def __init__(self):
        self.my_nd = Text = NSWorkspace.sharedWorkspace().activeApplication()['NSApplicationName']
        # set_forground()
        self.check_bonus = 1
        self._stop = False
        self.stop_move = 0
        ...
        while True:
            try:
                Text = NSWorkspace.sharedWorkspace().activeApplication()['NSApplicationName']  # 获取当前活动应用标题
                window = pygetwindow.getWindowGeometry(Text)
                left = window[0]
                top = window[1]
                width = window[2]
                height = window[3]
                ...
                log.info("xx: " + str(self.xx) + " yy: " + str(self.yy))
    ...
    def use_it(self, x, y):
        if x != 1 or y != 1:
            self.click((0.8469 - 0.0787 * (x - 1), 0.7806 - 0.1723 * (y - 1)))
            time.sleep(0.5)
        # 点击使用
        self.click((0.1641, 0.0833))
        self.wait_fig(lambda: not self.check("yes1", 0.3641, 0.2259), 1.2)
        # 点击确认
        self.click((0.3641, 0.2259))
        r = self.wait_fig(lambda: not self.check("use_replace", 0.5312, 0.7231), 0.8)
        if r:
            # 覆盖效果
            self.click((0.3641, 0.2417))

    # 使用x排，y列的消耗品
    def use_consumable(self, x=1, y=1):
        self.press("l")
        if self.wait_fig(lambda: not self.check("use_atk", 0.4224, 0.0769), 3):
            time.sleep(0.4)
            self.click((0.4224, 0.0769))
            time.sleep(0.4)
            self.get_screen()
            if self.wait_fig(lambda: not self.check("use_star", 0.8229, 0.8306, threshold=0.9), 0.8):
                self.use_it(x, y)
                if self.wait_fig(lambda: not self.check("use_def", 0.3641, 0.0769), 2.2):
                    time.sleep(0.4)
                    self.click((0.3641, 0.0769))
                    time.sleep(0.4)
                    self.get_screen()
                    if self.wait_fig(lambda: not self.check("use_star", 0.8229, 0.8306, threshold=0.9), 0.6):
                        self.use_it(x, y)
                        self.wait_fig(lambda: not self.check("use_package", 0.9214, 0.9222), 2)
                        time.sleep(0.3)
                    self.press("c")
            else:
                self.press("c")
    ...
    def drag(self, pt1, pt2):
        ...
        time.sleep(0.2)
        pyautogui.drag(x2 - x1, y2 - y1, 0.4, button="left")
        time.sleep(0.3)
    ...
    def get_local(self, x, y, size, large=True):
        ...
        return self.screen[
               int(max(0, by - sx // 2)): int(min(self.yy, by + sx // 2)),
               int(max(0, bx - sy // 2)): int(min(self.xx, bx + sy // 2)),
               :,
               ]

    def get_local_loc(self):
        center = (217, 172)
        shape = (self.map_size, self.map_size)
        return self.screen[
               int(center[0] - 0.5 * shape[0]): int(center[0] + 0.5 * shape[0]),
               int(center[1] - 0.5 * shape[1]): int(center[1] + 0.5 * shape[1]),
               :,
               ]
    ...
    def check(self, path, x, y, mask=None, threshold=None, large=True):
        ...
        if path == './imgs/run.jpg':
            local_screen = cv.cvtColor(local_screen, cv.COLOR_BGR2GRAY)
            local_screen[local_screen >= 210] = 255
            local_screen[local_screen < 210] = 0
    ...
    def exist_minimap(self):
        self.get_screen()
        local_screen = self.get_local_loc()
        blue = np.array([249, 194, 89])
        local_screen[np.sum(np.abs(local_screen - blue), axis=-1) <= 60] = blue

        self.loc_scr = local_screen

    # 从全屏截屏中裁剪得到游戏窗口截屏
    def get_screen(self):
        screenShot = self.sct.grab((self.x0, self.y0, self.x1, self.y1))
        img = Image.frombytes(
            'RGB',
            (screenShot.width, screenShot.height),
            screenShot.rgb,
        )
        self.screen = cv.cvtColor(np.array(img), cv.COLOR_BGR2RGB)
        return self.screen
    ...
    def take_fine_minimap(self, n=5, dt=0.01, dy=200):
        ...
        for i in range(n):
            ...
            pyautogui.drag(0, -dy, button="left")
            self.get_screen()
            self.exist_minimap()
    ...
    def get_bw_map(self, gs=1, local_screen=None):
        ...
        b_map = deepcopy(bw_map)
        b_map[
            np.sum((local_screen - gray) ** 2, axis=-1) <= 4800 + self.find * 2400
            ] = 255
        ...
        kernel = np.zeros((11, 11), np.uint8)  # 设置kenenel大小
        kernel += 1
        b_map = cv.dilate(b_map, kernel, iterations=1)
        bw_map[
            (np.sum((local_screen - white) ** 2, axis=-1) <= 1600 + self.find * 800)
            & (b_map > 200)
            ] = 255
        # 再次精确裁剪，这里区别模式只是防止bug，find=1时的裁剪是最精确的（中心点即为人物坐标）
        if self.find == 0:
            bw_map = bw_map[
                     int(shape[0] * 0.5) - self.radius: int(shape[0] * 0.5) + self.radius,
                     int(shape[1] * 0.5) - self.radius: int(shape[1] * 0.5) + self.radius,
                     ]
        else:
            bw_map = bw_map[
                     int(shape[0] * 0.5) - self.radius: int(shape[0] * 0.5) + self.radius,
    ...
    def get_now_direc(self, loc_scr):
        ...
        lower = np.array([90, 110, 198])  # 90 改成120只剩箭头，但是角色移动过的印记会消失
    ...
    def goodf(self):
        if not self.check("f", 0.4016, 0.3935, mask="mask_f1"):
            return False
    ...
    def move_to_interac(self, ii=0, abyss=0):
        ...
        min_val, max_val, min_loc, max_loc = cv.minMaxLoc(result)
        if max_val > threshold:
            nearest = (max_loc[1] + sp[0] // 2 + 2, max_loc[0] + sp[1] // 2 + 2)
            target = (nearest, 1)
        ...
        for i in range(local_screen.shape[0]):
            for j in range(local_screen.shape[1]):
                if self.get_dis((self.radius, self.radius), (i, j)) >= self.radius:
                    local_screen[i, j] = [0, 0, 0]
        if max_val <= threshold:
            red = [61, 68, 210]
            rd = np.where(np.sum((local_screen - red) ** 2, axis=-1) <= 50)
            if rd[0].shape[0] > 0:
        ...
        if target[1] >= 1:
            self.ang = self.get_now_direc(local_screen)
            ang = (
    ...
    def get_direc(self):
        ...
        blue = np.array([249, 194, 89])
        white = np.array([210, 210, 210])
        red = np.array([49, 49, 140])
        yellow = np.array([145, 192, 220])
        black = np.array([0, 0, 0])
        shape = (int(self.scx * self.map_size), int(self.scx * self.map_size))
        bw_map = self.get_bw_map(gs=1)
        self.loc_off = 0
        tm = time.time()
        self.get_loc(bw_map, rg=50 - self.find * 10)
        if self.find == 1:
            self.press("w", 0.2)
        self.get_screen()
        local_screen = self.get_local_loc()
        local_screen[np.sum(np.abs(local_screen - blue), axis=-1) <= 100] = blue
        # 录图模式，将小地图覆盖到录制的大地图中
        if self.find == 0:
        ...
        # 寻路模式
            ...
            time.sleep(0.5)
            bw_map = self.get_bw_map(gs=1)
            self.get_loc(bw_map, rg=50, offset=self.get_offset(4))
            # self.get_loc(bw_map, rg=10)
            self.get_real_loc(1)
            # 复杂的定位、寻路过程
            ...
            for i in range(3000):
                ...
                self.get_loc(bw_map, fbw=1, offset=self.get_offset(2 + (c <= 2)), rg=20 + 12 * (c <= 2))
                # self.get_loc(bw_map, fbw=1, rg=10)
                self.get_real_loc(2)
                ang = (
                        math.atan2(loc[0] - self.real_loc[0], loc[1] - self.real_loc[1])
                        / math.pi
                ...
                if self.debug:
                    self.big_map[
                    self.real_loc[0] - 1: self.real_loc[0],
                    self.real_loc[1] - 1: self.real_loc[1],
                    ] = 49
                    # 轨迹图
                ...
                if dls[0] <= nds:
                    ...
                    if t > 0:
                        ...
                        self.get_loc(bw_map, rg=20, fbw=1)
                        self.get_real_loc()
                        self.move = 0
                        time.sleep(0.12)
                        t -= 1
                ...
                if nds <= ps[type]:
                    ...
                    self.get_screen()
                    if type == 3 and self.check("f", 0.4016, 0.3935, mask="mask_f1"):
                        self.press('f')
                        keyops.keyUp("w")
    ...
    # 视角转动x度
    def mouse_move(self, x, fine=0.5):
        if x > 30 // fine:
            y = 30 // fine
        elif x < -30 // fine:
            y = -30 // fine
        ...
        if self._stop == 0 and self.stop_move == 0:
            pyautogui.moveTo(0.5 * (self.x0 + self.x1), 0.5 * (self.y0 + self.y1))
            pyautogui.drag(dx, 0, 0.2, button="left")
    ...
    def write_map(self, bw_map):
        for i in range(bw_map.shape[0]):
            for j in range(bw_map.shape[1]):
                ...
                if bw_map[i, j] == 255:
                    if (
                            self.big_map[self.now_loc[0] - self.radius + i, self.now_loc[1] - self.radius + j]
                            < 250
                    ):
                        self.big_map[
                            self.now_loc[0] - self.radius + i, self.now_loc[1] - self.radius + j
                        ] += 50
    ...
    def get_loc(self, bw_map, rg=10, fbw=0, offset=None):
        ...
        if self.find and fbw == 0:
            tbw = cv.resize(bw_map, (2 * self.radius + tt * 2, 2 * self.radius + tt * 2))
            tbw[tbw > 150] = 255
            tbw[tbw <= 150] = 0
            tbw = tbw[tt: 2 * self.radius + tt, tt: 2 * self.radius + tt]
            bo_2 = tbw == 255
        ...
        bo_4 = (b_map != 0) & (bo_1 == 0)
        # 枚举匹配，找到匹配点最多的坐标
        for i in range(rge * 2 - 2 * self.radius):
            for j in range(rge * 2 - 2 * self.radius):
                if (i - rge + self.radius) ** 2 + (j - rge + self.radius) ** 2 > rg ** 2:
                    continue
                p = 2 * np.count_nonzero(bo_3[i: i + 2 * self.radius, j: j + 2 * self.radius] & bo_1)
                p += np.count_nonzero(bo_3[i: i + 2 * self.radius, j: j + 2 * self.radius] & bo_4)
                if p > max_val:
                    max_val = p
                    max_loc = (i, j)
                    if self.debug == 2:
                        tmp = np.zeros((2 * self.radius, 2 * self.radius), dtype=np.uint8)
                        tpp = bo_3[i: i + 2 * self.radius, j: j + 2 * self.radius]
                        tmp[tpp != 0] = 255
                        tmp[bo_1 != 0] = 150
                        tmp[bo_4 != 0] = 50
                if self.find and fbw == 0:
                    p = 2 * np.count_nonzero(bo_3[i: i + 2 * self.radius, j: j + 2 * self.radius] & bo_2)
                    p += np.count_nonzero(bo_3[i: i + 2 * self.radius, j: j + 2 * self.radius] & bo_5)
                    if p > max_val:
                        max_val = p
                        max_loc = (i, j)
        if max_val != 0:
            self.now_loc = (
                max_loc[0] + self.radius - rge + self.now_loc[0],
                max_loc[1] + self.radius - rge + self.now_loc[1],
            )
        if self.debug == 2:
    ...
    def get_real_loc(self, delta=0):
        ...
        self.real_loc = (int(x + dx), int(y + dy))
    ...
    def get_map(self):
        ...
        cv.imwrite(self.map_file + "target.jpg", cv.cvtColor(tp, cv.COLOR_GRAY2BGR))
    ...
    def print_stack(self, num=1):
        if self.debug:
            ...
            for i in range(num):
                try:
                    print(stk[-2].name, stk[-3 - i].filename.split('\\')[-1].split('.')[0], stk[-3 - i].name,
                          stk[-3 - i].lineno)
                except:
                    pass

    def check_auto(self):
        auto = self.check("z", 0.1005, 0.9176, large=False, mask="mask_auto")
        cvt = cv.cvtColor(auto, cv.COLOR_BGR2HSV)
        lower = np.array([22, 58, 200])
    ...
    def isrun(self):
        ...
        loc_scr = self.get_local(0.9108, 0.8028, shape)
        hsv = cv.cvtColor(loc_scr, cv.COLOR_BGR2HSV)  # 转HSV
        lower = np.array([90, 90, 225])  # 90 改成120只剩箭头，但是角色移动过的印记会消失
        upper = np.array([255, 255, 255])
        mask = cv.inRange(hsv, lower, upper)  # 创建掩膜
        sum = np.sum(mask)
        ...
        res = self.check('run', 0.8484, 0.7019, threshold=0.9) and sum > 40000 and sum < 65000
        if self.tm > 0.96:
            res = 1
    ...
    def get_direc_only_minimap(self):
        log.info("mini_state: " + str(self.mini_state) + ", floor: " + str(self.floor))
        if self.debug == 2:
                ...
                if self.check("z", 0.5000, 0.9074, mask="mask_z", threshold=0.90):
                    ...
                    if self.mini_state == 1 and self.floor == 12:
                        ...
                        for i in range(4):
                            ...
                            time.sleep(1.5)
                            self.get_screen()
                            if not self.check("z", 0.5000, 0.9074, mask="mask_z", threshold=0.90):
                                break
                            if self._stop:
                                break
                        keyops.keyDown("w")
                    iters = 0
                    while self.check("z", 0.5000, 0.9074, mask="mask_z", threshold=0.90) and not self._stop:
                        iters += 1
                        if iters > 4:
                            break
                        self.click((0.1833, 0.2343))
